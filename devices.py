...
class Devices:
    ...
    make_siggen(self, device_id, signal_pattern): Make a signal generator with
                                                  the specified signal pattern.
                                                  signal_pattern is a stream
                                                  of 0s and 1s, specifying the
                                                  binary waveform. 
    ...
    def __init__(self, names):
        ...
        self.device_types = [self.CLOCK,
                             self.SWITCH, self.D_TYPE,
    ...
    def make_clock(self, device_id, clock_half_period):
        ...
        # clock initialised to a random point in its cycle
        self.cold_startup()

    def make_siggen(self, device_id, signal_pattern):
        """Make a signal generator with the specified signal pattern.

        signal_pattern is a stream of 0s and 1s, specifying the
        binary waveform.
        """
        self.add_device(device_id, self.SIGGEN)
        device = self.get_device(device_id)
        device.signal_pattern = []
        for i in signal_pattern:
            if i == '0':
                device.signal_pattern.append(self.LOW)
            else:
                device.signal_pattern.append(self.HIGH)
        # siggen initialised to a random point in its sequence
        self.cold_startup()
    ...
    def cold_startup(self):
        ...
        for device in self.devices_list:
            if device.device_kind == self.D_TYPE:
                ...
                i = random.randrange(0, len(device.signal_pattern))
                sig = device.signal_pattern[i]
                self.add_output(device.device_id, output_id=None,
                                signal=sig)
                device.signal_pattern_index = i
    ...
    def make_device(self, device_id, device_kind, device_property=None):
        ...
        if self.get_device(device_id) is not None:
            ...
            # Device property is the signal pattern
            bStr = isinstance(device_property, str)
            if device_property is None:
                error_type = self.NO_QUALIFIER
            elif not bStr:
                error_type = self.INVALID_QUALIFIER
                ...
                bMake = True
                # Check device property is made up of only ones and zeros
                for i in device_property:
                    if not (i == '0' or i == '1'):
                        error_type = self.INVALID_QUALIFIER
                        bMake = False
                if bMake:
