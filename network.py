...
class Network:
    ...
    execute_clock_siggen(self, device_id): Simulates a clock and updates its
                                           output signal value.

    update_clocks(self): If it is time to do so, sets clock signals to RISING
                         or FALLING.

    update_siggen(self): If it is time to do so, set siggen signals to RISING
                         or FALLING.
    ...
    def execute_clock_siggen(self, device_id):
        """Simulate a clock/siggen and update its output signal value.
    ...
    def update_siggen(self):
        ...
        for device_id in siggen_devices:
            device = self.devices.get_device(device_id)
            period_length = len(device.signal_pattern)

            # Check where siggen is in its sequence
            current_index = device.signal_pattern_index
            next_index = current_index + 1
            if next_index == period_length:
                next_index = 0

            output_signal = device.signal_pattern[current_index]
            next_signal = device.signal_pattern[next_index]
            if (output_signal == self.devices.HIGH and
                    next_signal == self.devices.LOW):
                device.outputs[None] = self.devices.FALLING
            elif (output_signal == self.devices.LOW and
                  next_signal == self.devices.HIGH):
                    device.outputs[None] = self.devices.RISING

            device.signal_pattern_index = next_index
    ...
    def execute_network(self):
        ...
        # This sets clock/siggen signals to RISING or FALLING, where necessary
        self.update_clocks()
        self.update_siggen()
